...
-- Path: ReplicatedStorage.Systems.PlayersSystem.ChatCommands.Callbacks.Damage
-- Source: decompile
...
return {
    ...
    ["run"] = function(u2, p3, u4) --[[ Name: run, Line 9 ]]
        ...
        local v7, v8 = pcall(function() --[[ Line: 11 ]]
            ...
            u1.server_damageCharacter(u2.Character, "command", v6)
        end)
        if v7 then
            p3("Damaged")
        else
            p3("Error using command. Example: /damage 5", Color3.fromRGB(255, 0, 0))
            warn(v8)
        end
    end
}